<?php
     ...
     */
    /*
    Plugin Name: POWr Map
    Plugin URI: https://www.powr.io/plugins/map
    Description: Get more visitors with a custom map  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////Create Map widget/////////////////////////////////
    class Powr_Map extends WP_Widget{
      public $plugin;
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_map',
                             __( 'POWr Map' ),
                             array( 'description' => __( 'Map by POWr.io') )
        );
      ...
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-map' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Map.
        </p>
        <p>
    ...
    function register_powr_map() {
      register_widget( 'Powr_Map' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_map' );
    //Create short codes for adding plugins anywhere:
    function powr_map_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-map' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-map' label='$label'></div>";
      }else{
      	"<div class='powr-map'></div>";
      }
    }
    add_shortcode( 'powr-map', 'powr_map_shortcode' );
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_map_menu' );
    function powr_map_menu() {
      add_menu_page( 'POWr Map', 'POWr Map', 'manage_options', 'powr-map-settings', 'powr_map_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_map_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/map';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/map';
      }
    ...
    register_activation_hook(__FILE__, 'powr_map_plugin_activate');
      add_action('admin_init', 'powr_map_plugin_redirect');

      function powr_map_plugin_activate() {
      add_option('powr_map_plugin_do_activation_redirect', true);
      }
      ...
      function powr_map_plugin_redirect() {
      if (get_option('powr_map_plugin_do_activation_redirect', false)) {
          delete_option('powr_map_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-map-settings&' );
          }
       }
      }
